...
def makeImage(simulname,index):
    coordCirc = pd.read_excel('coord.xlsx',sheet_name=0)
    coordEll = pd.read_excel('coord.xlsx',sheet_name=1)

    simData = pd.read_excel('simulation_results/' + simulname + '_injury_analysis.xlsx', sheet_name=index, header=None, names=['Name', 'Fill', 'Prob'])
